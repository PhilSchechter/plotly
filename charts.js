...
// Demographics Panel + washfreq gauge
...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    //var yticks = ids.map(tick => ids).slice(0,10);
    ...
    var barLayout = {
      title: 'belly bug population'
    };

    // 10. Use Plotly to plot the data with the layout. 
    //var barPlotArea = d3.select("#bar");
    ...
    d3.json("samples.json").then((data) => {
      ...
      var gaugeData = [{
        domain: { x: [0, 1], y: [0, 1] },
        title: { text: "Weekly Belly Scrubs" },
        value: wfreq,
        type: 'indicator',
        mode: 'gauge+number',
        gauge: {
